...
public class RegisterController {
	...
	void btnRegister_onClick(ActionEvent event) {
		if (txtUsername.getText().isEmpty() || txtEmail.getText().isEmpty() || txtPass.getText().isEmpty()
				|| txtCPass.getText().isEmpty()) {
		...
		if (ub.GetUserByUsername(new GetUserByUsernameRequest(txtUsername.getText())).size() > 0) {
			AlertManager.ShowAlert(AlertType.ERROR, "Hata!",
					"Ayný kullanýcý adýna sahip baþka bir kullanýcý bulunmakta lütfen kullanýcý adýnýzý deðiþtiriniz!!");
			imgLogo.setImage(new Image("http://epetiste-001-site1.etempurl.com/javafx/warning.png"));
			return;
		}
		...
		ub.AddUser(request);
		AlertManager.ShowAlert(AlertType.CONFIRMATION, "Kayýt iþlemi", "Kayýt iþlemi baþarýlý!");
		StageManager.Close(btnRegister.getScene());
	}
